    ...
    <aside className="hidden md:flex w-64 bg-white shadow-xl fixed left-0 top-16 bottom-0 p-5 flex-col z-40">
        ...
        {navItems.map((item) => {
          ...
          return (
            <Link
              key={item.path}
              to={item.path}
              className={`group flex items-center gap-3 px-4 py-2.5 rounded-xl transition-all duration-200 font-medium text-sm ${
                isActive
      ...
      <div className="mt-auto pt-4 border-t border-gray-100">
        ...
        >
          <MessageCircle
            className={`w-5 h-5 flex-shrink-0 transition-colors duration-200 ${
              location.pathname === '/feedback' ? 'text-white' : 'text-pink-500 group-hover:text-white'
            }`}
            strokeWidth={2}
          />
          Danos tu opinión
