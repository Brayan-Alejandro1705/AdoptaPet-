  ...
  return (
    <div className="flex flex-col h-full min-h-0 bg-white shadow-lg rounded-xl overflow-hidden">
      <header className="bg-[#f0f2f5] px-4 py-3 flex items-center gap-3 border-b border-gray-200 flex-shrink-0">
        {onBack && (
          <button onClick={onBack} className="md:hidden text-gray-600 hover:text-gray-800 -ml-2">
                ...
                <div className={`rounded-lg shadow-sm ${isMe ? 'bg-purple-200' : 'bg-white'}`}
                  ...
                  <div className="flex items-end gap-2">
                    ...
                    <div className="flex items-center gap-1 shrink-0">
                      ...
                      {isMe && (
                        <svg className={`w-4 h-4 ${read ? 'text-purple-800' : 'text-gray-500'}`} viewBox="0 0 16 15" fill="none" aria-label={read ? 'Leído' : 'Enviado'}>
